...
const groupsData = [
  { 
    ...
    codes: ['HK', 'MM', 'POC', 'CIN', 'KUR', 'TUX', 'GUD', 'LTS', 'KER', 'BAD', 'RUR', 'CHO'] 
  },
  { 
    name: 'Monsters', 
    color: 'bg-purple-500',
    borderColor: 'border-purple-500',
    codes: ['KAW-MON', 'LAB', 'ZIM', 'MOK', 'PAT', 'SPO', 'TYC'] 
  },
  { 
    name: 'Plushies', 
    color: 'bg-purple-500',
...
const groups = sortAlphabetically(groupsData).filter(g => g.name !== 'Hello Kitty');
...
export default function KawaiiPage() {
    ...
    async function fetchAllStickers() {
      ...
      for (const group of groups) {
        for (const code of group.codes) {
          try {
            const stickers = await loadStickersBySubcategory(code);
            if (!data[group.name]) {
              data[group.name] = [];
            }
            data[group.name].push(...stickers);
          } catch (err) {
            console.error(`Error loading stickers for ${code}:`, err);
          }
        }
      }
  ...
  return (
    <div className="min-h-screen bg-perforated text-white font-orbitron flex flex-col items-center p-4 pt-4 landscape:pt-2 pb-16">
      ...
      {/* Category Title */}
      <div className="text-center mb-2 landscape:mb-1">
        <h1 className="text-purple-500 text-lg font-bold font-audiowide animate-categoriesFlicker">
          Kawaii
        </h1>
      </div>
      ...
      >
        ...
        {groups.map((group, index) => (
          <button
            key={index}
            className={`inline-block rounded-full ${group.color} px-4 py-2 mx-1 font-montserrat hover:scale-105 transition-transform`}
            style={{ color: 'black', backgroundColor: '#a855f7' }}
          >
            {group.name}
          </button>
        ))}
      ...
      {/* Kawaii Stickers Grid - two boxes matching button colors */}
      <div className="w-full">
        <div className="flex justify-center pb-4 landscape:pb-16">
          <div className="grid grid-cols-1 landscape:grid-cols-2 md:grid-cols-1 md:landscape:grid-cols-2 gap-3 landscape:gap-4 md:gap-5 max-w-lg landscape:max-w-4xl md:max-w-2xl md:landscape:max-w-4xl px-4">
            {groups.map((group, i) => {
              const stickers = stickersByCode[group.name] || [];
              
              return (
                <div key={i} className={`w-52 h-52 landscape:w-52 landscape:h-52 md:w-56 md:h-56 md:landscape:w-56 md:landscape:h-56 border-4 border-purple-500 flex items-center justify-center overflow-hidden`}>
                  {isLoading ? (
                    ...
                    <span className="text-gray-500 text-sm text-center px-2">{group.name}</span>
                  )}
                </div>
              );
            })}
